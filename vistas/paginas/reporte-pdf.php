<?php 
	...
	if (isset($archivo['idPersonal']) && !empty($archivo['idPersonal'])) {
		...
		$nombreArchivo = 'horario-docente.php';		
	}else if (isset($archivo['idSeccion']) && !empty($archivo['idSeccion'])) {
		$idSeccion = $archivo['idSeccion'];
		$idPeriodo = $_SESSION['idPeriodo'];
		$modeloDocenteHorario = new ModeloCursoHorario();
		$horario = $modeloDocenteHorario->mdlSeccionHorario($idSeccion, $idPeriodo);
		$modeloAula = new modeloAula();
		$seccion = $modeloAula->mdlDetallesSeccion($idSeccion);
		if ($seccion['turno'] == 'M') {
		...
		$titulo = $seccion['nombreCarrera'].' '.$seccion['nombreSeccion'].' '.$seccion['nombreSede'].' TURNO '.$seccion['turno'];
		$nombreArchivo = 'horario-seccion.php';
	}else{
		header('location: inicio');
		return;
	}
	...
	ob_start();
	include('vistas/paginas/pdf/'.$nombreArchivo);
	$html = ob_get_clean();
